      ...
      <footer className="border-t border-slate-900 bg-slate-950 py-8 text-center text-xs text-slate-500">
        <div className="max-w-7xl mx-auto px-4 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-4 text-slate-400">
            <button onClick={() => setCurrentSection('home')} className="hover:text-white transition-colors">{settings.footerStoreText ?? 'Store'}</button>
            <button onClick={() => setCurrentSection('lookup')} className="hover:text-white transition-colors">{settings.footerTrackText ?? 'Track Order'}</button>
            <button onClick={() => setCurrentSection('faq')} className="hover:text-white transition-colors">{settings.footerFaqText ?? 'FAQ'}</button>
            <button onClick={() => setCurrentSection('contact')} className="hover:text-white transition-colors">{settings.footerSupportText ?? 'Support'}</button>
          </div>
        </div>
      </footer>
