...
export default function App() {
  ...
  const handleOrderSuccess = (order: Order) => {
    ...
    const shouldSendWithEmailJs = (
      settings.emailDeliveryEnabled === true &&
      (settings.emailDeliveryProvider ?? 'emailjs') === 'emailjs' &&
      order.orderType !== 'prebooking' &&
      /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(customerEmail) &&
      order.licenseKey !== 'NO-KEY-AVAILABLE-CONTACT-ADMIN'
    );

    // Google Apps Script delivery is intentionally not called from the app.
    // Its private, scheduled Gmail script reads this pending order from Firebase
    // and sends the email without needing an EmailJS domain whitelist.
    if (!shouldSendWithEmailJs) return;

    // Email is intentionally sent after the order record has been created, so
    // the buyer can still copy the key on-screen even if their inbox is slow.
    void sendLicenseKeyEmail(settings, order)
      .then(async () => {
        const emailSentAt = Date.now();
        setLastOrder(current => current?.id === order.id
          ? { ...current, emailDeliveryStatus: 'sent', emailSentAt }
          : current);
        await updateData(`orders/${order.id}`, { emailDeliveryStatus: 'sent', emailSentAt });
      })
      .catch(async (error) => {
        console.error('License email delivery failed:', error);
        setLastOrder(current => current?.id === order.id
          ? { ...current, emailDeliveryStatus: 'failed' }
          : current);
        try {
          await updateData(`orders/${order.id}`, { emailDeliveryStatus: 'failed' });
        } catch (saveError) {
          console.error('Could not record email delivery status:', saveError);
        }
      });
  };
  ...
  const isMaintenanceScreen = showUserPanel && settings.maintenanceEnabled === true && currentSection !== 'admin' && currentSection !== 'admin-login';
  ...
  const maintenanceMessage = isNativeApp
    ? 'The NovaEsp Shop app is currently being updated. Please check back soon.'
