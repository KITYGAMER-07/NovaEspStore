...
export const StoreFront: React.FC<StoreFrontProps> = ({
  ...
  const handleInitiatePayment = () => {
    ...
    if (isPreBooking && !preBookingOpen) {
      alert('Pre-booking is currently closed or sold out.');
      return;
    }
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="text-[9px] font-extrabold uppercase tracking-widest bg-amber-400 text-slate-950 px-2 py-1 rounded">{activePreBooking.badgeText || 'PRE-BOOKING'}</span>
                        <span className="text-xs font-bold text-white">{activePreBooking.title || 'Reserve this product early'}</span>
                      </div>
                      <p className="text-[11px] text-slate-400 mt-2">{activePreBooking.description || 'Pay the full amount now and reserve your release.'}</p>
                      {activePreBooking.endDate && <p className="text-[10px] text-amber-300 mt-1 font-bold">Closes in {preBookingCountdownText}</p>}
                      {Number(activePreBooking.slots || 0) > 0 && <p className="text-[10px] text-slate-500 mt-1">{Number(activePreBooking.slots)-Number(activePreBooking.bookedCount||0)} slots remaining</p>}
                    </div>
                    <button type="button" disabled={!preBookingOpen} onClick={() => setIsPreBooking(v => !v)} className={`shrink-0 px-3 py-2 rounded-xl text-[10px] font-extrabold ${isPreBooking ? 'bg-amber-400 text-slate-950' : 'bg-slate-800 text-slate-300'} disabled:opacity-40`}>{isPreBooking ? 'Pre-Booking Selected' : 'Pre-Book'}</button>
                  </div>
                  {isPreBooking && activePreBooking.rules && <div className="mt-3 p-3 rounded-xl bg-slate-950/60 border border-slate-800 text-[10px] text-slate-400 whitespace-pre-line">{activePreBooking.rules}</div>}
                </div>
              )}
                        ...
                        <div className="flex flex-col gap-4">
                          ...
                          <div className="space-y-2">
                            {trialApkLinks.map(link => (
                              <a key={link.id} href={link.url} target="_blank" rel="noopener noreferrer" className="w-full flex items-center justify-between gap-3 p-3 rounded-xl bg-slate-950/60 border border-slate-800 hover:bg-slate-900 transition-colors">
                                <div><span className="text-xs font-bold text-white">{link.name}</span>{link.version && <span className="ml-2 text-[9px] font-bold px-1.5 py-0.5 rounded bg-teal-500/20 text-teal-300 border border-teal-500/30">{link.version}</span>}</div><span className="shrink-0 px-3 py-1.5 rounded-lg bg-teal-400 text-slate-950 text-[10px] font-bold">Download APK</span>
                              </a>
                            ))}
                            {trialApkLinks.length === 0 && apkUrl && (
                              <a href={apkUrl} target="_blank" rel="noopener noreferrer" className="w-full flex items-center justify-between gap-3 p-3 rounded-xl bg-slate-950/60 border border-slate-800 hover:bg-slate-900 transition-colors">
                                <div><span className="text-xs font-bold text-white">{apkAppName}</span><span className="ml-2 text-[9px] font-bold px-1.5 py-0.5 rounded bg-teal-500/20 text-teal-300 border border-teal-500/30">{apkVersion}</span></div><span className="shrink-0 px-3 py-1.5 rounded-lg bg-teal-400 text-slate-950 text-[10px] font-bold">Download APK</span>
                              </a>
                            )}
                          </div>
                        </div>
                      </motion.div>
                      {setupChannelUrl?.trim() && <a href={setupChannelUrl.trim()} target="_blank" rel="noopener noreferrer" className="w-full mb-6 flex items-center justify-center gap-2 px-4 py-3 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-bold text-xs border border-emerald-400 transition-all shadow-lg shadow-emerald-500/20">Setup Guide</a>}
                    </>
                  );
                })()}
