...
export const StoreFront: React.FC<StoreFrontProps> = ({
  ...
  const [showTrialRules, setShowTrialRules] = useState<boolean>(false);
  ...
  // Lock the trial-success background while Rules are open. This prevents
  // accidental page scrolling behind the Rules dialog, especially on mobile.
  useEffect(() => {
    if (!showTrialRules) return;
  ...
  return (
    <div className="relative py-8 sm:py-12 overflow-x-hidden">
      ...
      {showDemoGatewayModal && (
        <div className="fixed inset-0 z-50 flex items-start sm:items-center justify-center overflow-y-auto p-4 bg-slate-950/80 backdrop-blur-md animate-fadeIn overscroll-contain">
      ...
      {showTrialModal && (
        <div className="fixed inset-0 z-50 flex items-start sm:items-center justify-center overflow-y-auto p-4 bg-slate-950/80 backdrop-blur-md animate-fadeIn overscroll-contain">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 sm:p-8 max-w-lg w-full max-h-[calc(100dvh-2rem)] overflow-y-auto overscroll-contain shadow-2xl relative my-2 sm:my-4">
              ...
              >
                ...
                <div className="space-y-3">
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-2 py-3 rounded-xl bg-amber-500/10 hover:bg-amber-500/20 text-amber-300 font-extrabold text-xs border border-amber-500/30 transition-all active:scale-[0.98]"
                  >
                ...
                <AnimatePresence>
                  {showTrialRules && (
                    <motion.div
                      ...
                      animate={{ opacity: 1 }}
                      exit={{ opacity: 0 }}
                      className="fixed inset-0 z-[120] h-[100dvh] w-full bg-black/70 backdrop-blur-sm p-4 flex items-center justify-center overflow-hidden overscroll-none"
                    ...
                    >
                      <motion.div
                        ...
                        className="w-full max-w-lg max-h-[78dvh] overflow-y-auto overscroll-contain rounded-2xl bg-slate-950 border border-amber-500/30 shadow-2xl p-5 sm:p-6 text-left"
                      >
                        ...
                        <button
                          type="button"
                          onClick={() => setShowTrialRules(false)}
                          className="w-full mt-5 py-3 rounded-xl bg-slate-800 hover:bg-slate-700 text-white font-bold text-xs border border-slate-700 transition-all"
                        >
                          Close Rules
                        </button>
                      </motion.div>
