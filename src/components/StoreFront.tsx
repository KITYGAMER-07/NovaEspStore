...
export const StoreFront: React.FC<StoreFrontProps> = ({
  ...
  const handleRedeemTrialCode = async (e: React.FormEvent) => {
    ...
    if (matchedKey.used) {
      setTrialErrorMessage('This Trial Code has already been activated / used!');
      return;
    }

    // Mark key as used in Firebase
    await updateData(`trialKeys/${matchedKey.id}`, {
      used: true,
      usedByEmail: trialUserEmail.trim() || 'Trial User',
      usedAt: Date.now()
    });
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 sm:p-8 max-w-lg w-full shadow-2xl relative">
            ...
            {!claimedTrialResult ? (
              ...
              >
                ...
                {/* APK Download Box with logo.jpg */}
                <motion.div 
                  initial={{ opacity: 0, y: 15 }}
                  animate={{ opacity: 1, y: 0 }}
                  transition={{ delay: 0.45 }}
                  className="p-4 rounded-2xl bg-slate-950/60 border border-slate-800 mb-6 flex items-center justify-between gap-3"
                >
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl bg-slate-900 border border-slate-700/80 overflow-hidden shrink-0 flex items-center justify-center p-0.5 shadow-md">
                      <img 
                        src="/logo.jpg" 
                        alt="NovaEsp Logo" 
                    ...
                    <div>
                      <h4 className="text-xs font-bold text-white">{apkAppName}</h4>
                      <p className="text-[10px] text-slate-400">Version: {apkVersion} • Direct Android APK</p>
                    </div>
                  </div>
                  ...
                  <a
                    ...
                    className="px-4 py-2.5 rounded-xl bg-teal-400 hover:bg-teal-300 text-slate-950 text-xs font-bold flex items-center gap-1.5 transition-all shrink-0 shadow-lg shadow-teal-500/20"
                  >
