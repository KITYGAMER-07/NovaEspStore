...
export const AdminPortal: React.FC<AdminPortalProps> = ({
  ...
  // PRODUCT APK DOWNLOADS — one direct APK mapped to each product
  const handleUploadProductApk = async (file: File) => {
    ...
    if (!product) return;
    const oldLink = product.apkLinks?.[0];
    try {
      ...
      await updateData(`products/${product.id}`, { apkLinks: [link] });
      if (oldLink?.storagePath && oldLink.storagePath !== uploaded.path) {
        try { await deleteApkFromSupabase(oldLink.storagePath); } catch (e) { console.warn('Old product APK cleanup failed:', e); }
      }
      setApkLinkNameInput('');
      setApkLinkVersionInput('');
      alert(`${product.name} APK uploaded successfully.`);
    } catch (error: any) {
  ...
  const handleDeleteApkLink = async (product: Product, linkId: string) => {
    ...
    try {
      ...
      await updateData(`products/${product.id}`, { apkLinks: [] });
    } catch (error: any) {
        ...
        <div className="space-y-6">
          <div className="bg-slate-900/60 border border-slate-800 rounded-3xl p-6 space-y-5">
            <div>
              ...
              <p className="text-xs text-slate-400 mt-1">Select a product and upload its APK directly. Each product has one active APK; users only see the APK mapped to the product they purchased or activated in a trial.</p>
            </div>
            ...
            <label className="inline-flex items-center gap-2 px-4 py-3 rounded-xl bg-emerald-500 text-slate-950 text-xs font-black cursor-pointer hover:bg-emerald-400 transition">
              <Upload className="w-4 h-4" />
            ...
            <p className="text-[10px] text-slate-500">APK only · max 50 MB · direct upload to Supabase Storage. Uploading a replacement automatically makes the new APK the active file for that product.</p>
          </div>

          <div className="space-y-3">
            {products.map(product => {
              const link = product.apkLinks?.[0];
              return (
                    ...
                    <div className="min-w-0">
                      ...
                      {link ? (
                        <div className="mt-2">
                          ...
                          <p className="text-[10px] text-slate-500 mt-1 truncate">{link.fileName || 'Product APK'}</p>
                        </div>
                      ) : <p className="text-xs text-slate-500 mt-1">No APK uploaded for this product.</p>}
                    </div>
                    {link && <button onClick={()=>handleDeleteApkLink(product, link.id)} disabled={deletingProductApkId===product.id} className="shrink-0 px-3 py-2 rounded-xl bg-rose-500/10 border border-rose-500/20 text-rose-400 hover:bg-rose-500/20 disabled:opacity-50 text-xs font-bold">{deletingProductApkId===product.id ? 'Deleting...' : 'Delete APK'}</button>}
                  </div>
                </div>
              );
            })}
