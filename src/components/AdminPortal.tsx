...
export const AdminPortal: React.FC<AdminPortalProps> = ({
  ...
  const handleClearAllFilteredTrialKeys = async () => {
    const listToDelete = trialKeys.filter(k => (trialFilterTab === 'active' ? !k.used : k.used));
    if (listToDelete.length === 0) {
      alert(`No ${trialFilterTab === 'active' ? 'active' : 'claimed/used'} trial keys to delete.`);
      return;
    }
    const label = trialFilterTab === 'active' ? 'Active Trial Codes' : 'Claimed / Used Trials';
          ...
          <div className="bg-slate-900/60 border border-slate-800 rounded-3xl p-5 sm:p-6">
            <div className="flex flex-wrap items-center justify-between gap-4 mb-4">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-1">
                  Select a product, attach your actual license key, and click generate. The system creates a random Trial Code to share with users.
                </p>
              </div>
            </div>
            ...
            <div className="p-4 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 mb-6 space-y-3">
              ...
              <p className="text-[11px] text-slate-400">
                Enter the real key below. When you click generate, a random trial code (e.g., <span className="font-mono text-emerald-400">TRIAL-NOVA-8X92-A2K4</span>) will be generated for the user. When the user enters that trial code, they will receive this actual key!
              </p>
              <div className="flex flex-col sm:flex-row gap-2">
                <input
                  type="text"
          ...
          <div className="bg-slate-900/60 border border-slate-800 rounded-3xl p-5 sm:p-6">
            <div className="flex flex-wrap items-center justify-between gap-2 mb-4">
              <div className="flex flex-wrap items-center gap-2">
                ...
                >
                  Active Trial Codes ({trialKeys.filter(t => !t.used).length})
                </button>
                <button
                ...
                >
                  Claimed / Used Trials ({trialKeys.filter(t => t.used).length})
                </button>
              </div>

              {trialKeys.filter(k => (trialFilterTab === 'active' ? !k.used : k.used)).length > 0 && (
                <button
                ...
                >
                  <Trash2 className="w-3.5 h-3.5" />
                  <span>Clear All {trialFilterTab === 'active' ? 'Active' : 'Claimed'}</span>
                </button>
              )}
            </div>
            ...
            <div className="overflow-x-auto rounded-2xl border border-slate-800/80 bg-slate-950/40">
              <table className="w-full text-left text-xs">
                ...
                <tbody className="divide-y divide-slate-800/60 text-slate-300">
                  {trialKeys
                    .filter(k => (trialFilterTab === 'active' ? !k.used : k.used))
                    .map((k) => {
                      const prod = products.find(p => p.id === k.productId);
                      ...
                      return (
                        <tr key={k.id} className="hover:bg-slate-900/30 transition-colors">
                          ...
                          <td className="px-4 py-3.5 text-slate-400 whitespace-nowrap">
                            {k.used ? (
                              <span>{k.usedByEmail || 'Claimed user'} • {k.usedAt ? new Date(k.usedAt).toLocaleDateString() : 'N/A'}</span>
                            ) : (
                              <span className="text-emerald-400/80 font-semibold">Available for User</span>
                            )}
                          </td>
                          <td className="px-4 py-3.5 text-right whitespace-nowrap">
                            <button
