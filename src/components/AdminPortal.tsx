...
export const AdminPortal: React.FC<AdminPortalProps> = ({
  ...
  const handleReleaseBooking = async (order: Order) => {
    ...
    const keyObj = licenseKeys.find(k => k.productId === order.productId && k.durationId === order.durationId && !k.used);
    if (!keyObj) { alert('No license key is available for this booking yet.'); return; }
    await updateData(`licenseKeys/${keyObj.id}`, { used: true, usedAt: Date.now(), orderId: order.orderId });
        ...
        <div className="space-y-6">
          <div className="bg-slate-900/60 border border-slate-800 rounded-3xl p-5 sm:p-6">
            <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
              <div><h3 className="text-base font-bold text-white">Pre-Booking Manager</h3><p className="text-[11px] text-slate-500 mt-1">All fields are admin editable. Users always pay the full amount.</p></div>
              <button onClick={() => handleOpenPreBookingModal()} className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-amber-400 text-slate-950 text-xs font-bold hover:bg-amber-300"><Plus className="w-4 h-4"/> Add Pre-Booking</button>
            </div>
            <div className="overflow-x-auto rounded-2xl border border-slate-800/80 bg-slate-950/40">
              <table className="w-full text-left text-xs"><thead className="text-slate-400 uppercase bg-slate-900/80 border-b border-slate-800"><tr><th className="px-4 py-3">Product / Duration</th><th className="px-4 py-3">Status</th><th className="px-4 py-3">Slots</th><th className="px-4 py-3">Release</th><th className="px-4 py-3 text-right">Actions</th></tr></thead>
              <tbody className="divide-y divide-slate-800/60">{preBookings.map(pb => { const prod=products.find(p=>p.id===pb.productId); const dur=durations.find(d=>d.id===pb.durationId); const remaining=Number(pb.slots||0)>0 ? Number(pb.slots)-Number(pb.bookedCount||0) : null; return <tr key={pb.id}><td className="px-4 py-3"><div className="font-bold text-white">{prod?.name||'Unknown'}</div><div className="text-slate-500">{dur?.name||'Unknown'}</div></td><td className="px-4 py-3"><span className={pb.enabled!==false ? 'text-emerald-400':'text-slate-500'}>{pb.enabled!==false?'Enabled':'Disabled'}</span></td><td className="px-4 py-3 text-amber-300">{Number(pb.slots||0)>0 ? `${pb.bookedCount||0}/${pb.slots} (${remaining} left)` : `${pb.bookedCount||0} / Unlimited`}</td><td className="px-4 py-3 text-slate-300">{pb.releaseDate ? new Date(pb.releaseDate).toLocaleString() : 'TBA'}</td><td className="px-4 py-3 text-right"><button onClick={()=>handleOpenPreBookingModal(pb)} className="p-1.5 rounded-lg bg-slate-800 text-slate-300 mr-1"><Edit className="w-3.5 h-3.5"/></button><button onClick={()=>handleDeletePreBooking(pb.id)} className="p-1.5 rounded-lg bg-slate-800 text-rose-400"><Trash2 className="w-3.5 h-3.5"/></button></td></tr>})}</tbody></table>
            </div>
          </div>
          <div className="bg-slate-900/60 border border-slate-800 rounded-3xl p-5 sm:p-6">
            <h3 className="text-base font-bold text-white mb-4">Pre-Booking Payments & Status</h3>
            <div className="overflow-x-auto rounded-2xl border border-slate-800/80 bg-slate-950/40"><table className="w-full text-left text-xs"><thead className="text-slate-400 uppercase bg-slate-900/80 border-b border-slate-800"><tr><th className="px-4 py-3">Booking ID</th><th className="px-4 py-3">Customer</th><th className="px-4 py-3">Product</th><th className="px-4 py-3">Paid</th><th className="px-4 py-3">Status</th><th className="px-4 py-3">Release</th><th className="px-4 py-3 text-right">Actions</th></tr></thead><tbody className="divide-y divide-slate-800/60">{orders.filter(o=>o.orderType==='prebooking').map(o=><tr key={o.id||o.orderId}><td className="px-4 py-3 font-mono font-bold text-amber-300">{o.bookingId||o.orderId}</td><td className="px-4 py-3">{o.customerEmail}</td><td className="px-4 py-3 text-white">{o.productName}</td><td className="px-4 py-3 text-emerald-400 font-mono">₹{o.finalAmount}</td><td className="px-4 py-3 uppercase text-emerald-400 font-bold">{o.bookingStatus||'confirmed'}</td><td className="px-4 py-3">{o.releaseDate ? new Date(o.releaseDate).toLocaleString() : 'TBA'}</td><td className="px-4 py-3 text-right whitespace-nowrap"><button disabled={o.bookingStatus==='released'||o.bookingStatus==='cancelled'} onClick={()=>handleReleaseBooking(o)} className="px-2 py-1 rounded-lg bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 mr-1 disabled:opacity-40">Release</button><button disabled={o.bookingStatus==='cancelled'} onClick={()=>handleCancelBooking(o)} className="px-2 py-1 rounded-lg bg-rose-500/10 text-rose-400 border border-rose-500/20 disabled:opacity-40">Cancel</button></td></tr>)}</tbody></table></div>
          </div>
        </div>
      )}
