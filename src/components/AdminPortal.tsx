        ...
        <div className="space-y-8">
          ...
          <div className="flex items-center justify-between gap-3 mb-3">
            <div>
              ...
              <p className="text-[11px] text-slate-500 mt-1">Set starting values; future purchases and key deliveries update these numbers automatically.</p>
            </div>
            ...
            <div className="fixed inset-0 z-[100] bg-black/70 backdrop-blur-sm flex items-center justify-center p-4" onMouseDown={e => { if (e.target === e.currentTarget) setDashboardEditOpen(false); }}>
              <div className="w-full max-w-lg rounded-3xl border border-slate-800 bg-slate-900 shadow-2xl p-6">
                <div className="flex items-start justify-between gap-4 mb-5">
                  <div><h3 className="text-lg font-bold text-white">Edit Dashboard Statistics</h3><p className="text-xs text-slate-400 mt-1">These are starting values. New paid orders and key changes are automatically added/subtracted from them.</p></div>
      ...
      {activeTab === 'settings' && (
        <div className="bg-slate-900/60 border border-slate-800 rounded-3xl p-6 space-y-6 max-w-2xl">
          ...
          <div className="space-y-4 pt-2 border-t border-slate-800">
            <div>
              <label className="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-2">Direct APK Upload</label>
              <div className="rounded-2xl border border-slate-800 bg-slate-950/60 p-4 space-y-3">
                {apkStoragePath ? (
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    <div className="min-w-0">
                      <div className="flex items-center gap-2">
                        <span className="text-xs font-bold text-white truncate">{apkFileName || apkNameSetting || 'Uploaded APK'}</span>
                        <span className="text-[9px] font-bold px-1.5 py-0.5 rounded bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">UPLOADED</span>
                      </div>
                      {apkFileSize > 0 && <p className="text-[10px] text-slate-500 mt-1">{(apkFileSize / (1024 * 1024)).toFixed(2)} MB</p>}
                    </div>
                    <button
                      type="button"
                      onClick={handleDeleteUploadedApk}
                      disabled={deletingApk || uploadingApk}
                      className="inline-flex items-center justify-center gap-2 px-3 py-2 rounded-xl bg-rose-500/10 border border-rose-500/20 text-rose-400 hover:bg-rose-500/20 disabled:opacity-50 text-xs font-bold"
                    >
                      <Trash2 className="w-3.5 h-3.5" />
                  ...
                  <p className="text-xs text-slate-500">No direct APK file uploaded. You can upload an APK below or keep using a manual download URL.</p>
                )}

                <label className="inline-flex items-center gap-2 px-3 py-2.5 rounded-xl bg-emerald-500 text-slate-950 text-xs font-black cursor-pointer hover:bg-emerald-400 transition disabled:opacity-50">
                  <Upload className="w-4 h-4" />
                  {uploadingApk ? 'Uploading APK...' : apkStoragePath ? 'Replace APK' : 'Upload APK'}
                  <input
                    type="file"
                    ...
                    onChange={async e => {
                      ...
                      e.currentTarget.value = '';
                      if (!file) return;
                      await handleUploadApk(file);
                    }}
                  />
                </label>
                <p className="text-[10px] text-slate-500">APK only · max 50 MB · stored in Supabase Storage. Your &lt;20 MB APK is supported.</p>
              </div>
            </div>

            <div>
              <label className="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-1">Manual / GitHub APK Download URL (optional)</label>
              <input
                type="url"
                value={apkUrlSetting}
                ...
                className="w-full bg-slate-950 border border-slate-800 text-white text-xs rounded-xl p-3 focus:outline-none focus:border-emerald-500"
              />
              <p className="text-[10px] text-slate-500 mt-1">Uploading an APK automatically replaces this URL with the Supabase download URL.</p>
            </div>
