...
import { Product, Duration, Price, LicenseKey, TrialKey, Coupon, Order, Settings } from '../types';
import { 
  pushData, 
  updateData, 
  deleteData, 
...
export const AdminPortal: React.FC<AdminPortalProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<
    'dashboard' | 'products' | 'durations' | 'prices' | 'keys' | 'trials' | 'coupons' | 'orders' | 'settings'
  >('dashboard');
  ...
  const handleGenerateTrialKeyFromActualKey = async () => {
    ...
    // The Trial Code is exactly what the admin typed. No random code is generated.
    try {
      const duplicate = trialKeys.some(k => (k.trialCode || k.key || '').trim().toUpperCase() === manualTrialCode.toUpperCase());
      if (duplicate) {
  ...
  const handleSaveSettings = async () => {
    ...
    await setData('settings/setup', {
      channelUrl: setupChannelUrlSetting.trim()
    });
          ...
          <div className="bg-slate-900/60 border border-slate-800 rounded-3xl p-5 sm:p-6">
            ...
            <div className="p-4 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 mb-6 space-y-3">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-bold text-emerald-400 uppercase tracking-wider">Manual Trial Code & Actual Key</span>
              </div>
              <p className="text-[11px] text-slate-400">
                Enter the real key below. Enter the Trial Code manually below. Nothing is generated randomly. When users enter that exact Trial Code, they receive the attached actual license key. The same Trial Code can be reused by unlimited users!
              </p>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                <input
                  type="text"
                  placeholder="Enter Trial Code manually (e.g. MY-TRIAL-2026)"
                  value={trialCodeInput}
                  onChange={(e) => setTrialCodeInput(e.target.value)}
                  className="w-full bg-slate-950 border border-slate-800 text-white font-mono text-xs rounded-xl px-3.5 py-3 focus:outline-none focus:border-emerald-500 uppercase tracking-wide"
                />
                <input
                  type="text"
                  placeholder="Paste Actual Real Key (delivered to user)"
                ...
                <button
                  ...
                  className="px-6 py-3 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-extrabold text-xs shadow-lg shadow-emerald-500/20 transition-all shrink-0 flex items-center justify-center gap-2 sm:col-span-2"
                >
      ...
      {activeTab === 'settings' && (
        <div className="bg-slate-900/60 border border-slate-800 rounded-3xl p-6 space-y-6 max-w-2xl">
          ...
          <div className="space-y-2 pt-2 border-t border-slate-800">
            <div>
              <label className="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-1">Setup Channel Link</label>
              <input
                type="url"
                value={setupChannelUrlSetting}
                onChange={(e) => setSetupChannelUrlSetting(e.target.value)}
                placeholder="https://t.me/your_setup_channel"
                className="w-full bg-slate-950 border border-slate-800 text-white text-xs rounded-xl p-3 focus:outline-none focus:border-emerald-500"
              />
              <p className="text-[10px] text-slate-500 mt-1">This link opens when users press Setup Guide on Trial or Payment Success.</p>
            </div>
          </div>
