...
const PreBookingManagement: React.FC<PreBookingManagementProps> = ({
  preBookings, products, durations, orders, onAdd, onEdit, onDelete, onRelease, onCancel
}) => {
  ...
  return <div className="space-y-6">
    ...
    <div className="rounded-2xl border border-slate-800 bg-slate-900/60 overflow-hidden">
      ...
      <div className="overflow-x-auto"><table className="w-full min-w-[1050px] text-xs"><thead className="bg-slate-950/60"><tr className="text-left text-slate-500 uppercase tracking-wider"><th className="px-4 py-3">Booking ID</th><th className="px-4 py-3">User</th><th className="px-4 py-3">Product</th><th className="px-4 py-3">Amount</th><th className="px-4 py-3">Date</th><th className="px-4 py-3">Status</th><th className="px-4 py-3 text-right">Actions</th></tr></thead><tbody className="divide-y divide-slate-800">{visibleOrders.map(o=>{const st=o.bookingStatus||'confirmed'; return <tr key={o.id} className="text-slate-300 align-middle"><td className="px-4 py-3 font-bold text-white">{o.bookingId||o.orderId}</td><td className="px-4 py-3">{o.username||o.email||'—'}</td><td className="px-4 py-3">{productName(o.productId)}</td><td className="px-4 py-3">₹{Number(o.amount||0).toFixed(2)}</td><td className="px-4 py-3">{o.createdAt ? new Date(o.createdAt).toLocaleDateString() : '—'}</td><td className="px-4 py-3">{badge(st)}</td><td className="px-4 py-3"><div className="flex justify-end gap-2">{st!=='released'&&st!=='cancelled'&&<button onClick={()=>onRelease(o)} className="inline-flex items-center gap-1.5 px-3 py-2 rounded-lg bg-emerald-500/15 text-emerald-300 font-bold">Release</button>}{st!=='cancelled'&&st!=='released'&&<button onClick={()=>onCancel(o)} className="inline-flex items-center gap-1.5 px-3 py-2 rounded-lg bg-red-500/10 text-red-300 font-bold">Cancel</button>}</div></td></tr>})}{visibleOrders.length===0&&<tr><td colSpan={7} className="px-4 py-10 text-center text-slate-500">No pre-booking orders found.</td></tr>}</tbody></table></div>
    </div>
  </div>;
};
