...
export const AdminPortal: React.FC<AdminPortalProps> = ({
  ...
  // Keep the form fields synchronized with Firebase-loaded settings.
  // App loads settings asynchronously, so initializing these states only once
  // would leave the form with stale/default values.
  useEffect(() => {
  ...
  const handleSaveSettings = async () => {
    ...
    try {
      // Write all settings in one Firebase update so the button is atomic
      // from the user's point of view and no field is silently skipped.
      await updateData('settings', {
        razorpay: {
          keyId: rzpKeyIdInput.trim()
        },
        admin: {
      ...
      setSettingsSaveError(
        error instanceof Error
          ? error.message
          : 'Unable to save settings. Check Firebase Realtime Database permissions.'
      );
    }
  };
    ...
    <div className="py-8 max-w-7xl mx-auto px-4 sm:px-6">
      ...
      {activeTab === 'settings' && (
        <div className="bg-slate-900/60 border border-slate-800 rounded-3xl p-6 space-y-6 max-w-2xl">
          ...
          {settingsSaveError && (
            <div className="p-3 rounded-xl bg-red-500/10 border border-red-500/20 text-red-300 text-xs font-bold break-words">
