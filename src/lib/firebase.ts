...
export async function uploadWebsiteLogo(file: File): Promise<string> {
  ...
  const logoRef = storageRef(storage, `website/logo/${Date.now()}-${safeName}`);
  const uploaded = await uploadBytes(logoRef, file, { contentType: file.type, cacheControl: 'public,max-age=31536000' });
...
export async function uploadProductLogo(productId: string, file: File): Promise<string> {
  ...
  const safeName = file.name.replace(/[^a-zA-Z0-9._-]/g, '_');
  const logoRef = storageRef(storage, `website/product-logos/${productId}/${Date.now()}-${safeName}`);
  const uploaded = await uploadBytes(logoRef, file, { contentType: file.type, cacheControl: 'public,max-age=31536000' });
  return await getDownloadURL(uploaded.ref);
}
